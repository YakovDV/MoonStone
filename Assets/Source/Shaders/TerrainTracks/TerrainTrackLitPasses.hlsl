...
TEXTURE2D(_TrackMask);
SAMPLER(sampler_TrackMask);
...
#endif
{
    ...
    float2 trackUV = (IN.positionWS.xz - _TrackWorldMin.xy) * _TrackWorldInvSize.xy;

    half2 insideMinimum = step(0.0, trackUV);
    half2 insideMaximum = step(trackUV, 1.0);
    half insideTerrain = insideMinimum.x * insideMinimum.y * insideMaximum.x * insideMaximum.y;

    half trackMask = SAMPLE_TEXTURE2D(_TrackMask, sampler_TrackMask, saturate(trackUV)).r;
    half trackInfluence = saturate(trackMask * _TrackStrength) * insideTerrain;

    albedo = lerp(albedo, albedo * _TrackColor.rgb, trackInfluence);
